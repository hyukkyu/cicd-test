...
runWhenReady(initBoardDetailPage);

function initBoardDetailPage() {
...
function bindDetailEvents() {
  ...
  document.getElementById('detailReportButton')?.addEventListener('click', () =>
    openReportModal('POST', detailState.id, detailState.detail?.title)
  );
...
async function loadDetail() {
  setStatus(document.getElementById('detailStatus'), '게시글을 불러오는 중입니다...', 'info');
  try {
    const detail = await fetchJson(`/api/posts/${detailState.id}`);
    detailState.detail = detail;
    renderDetail(detail);
    setStatus(document.getElementById('detailStatus'), '', 'info');
    await loadDetailComments(true);
  } catch (error) {
    setStatus(document.getElementById('detailStatus'), error.message || '게시글을 불러오지 못했습니다.', 'error');
  }
}
...
function renderDetail(detail) {
  ...
  document.getElementById('detailLikeCount').textContent = `추천 ${detail.voteCount ?? detail.likeCount ?? 0}`;
  ...
  const attachments = Array.isArray(detail.fileUrls) ? detail.fileUrls : [];
...
async function voteDetail() {
  ...
  try {
    await fetchJson(`/api/posts/${detailState.id}/vote`, { method: 'POST' });
    await loadDetail();
  } catch (err) {
...
function renderDetailCommentItem(comment, depth) {
  ...
  const controls = [];
  controls.push(
    `<button class="btn btn-text btn-detail-reply" data-comment-id="${comment.id}" data-comment-author="${author}">답글</button>`
  );
  ...
  controls.push(
    `<button class="btn btn-text btn-report" data-report-type="COMMENT" data-report-id="${comment.id}" data-report-label="댓글 #${comment.id}">신고</button>`
  );
  item.innerHTML = `
...
function handleCommentListClick(event) {
  ...
  if (deleteBtn) {
    deleteBoardComment(Number(deleteBtn.dataset.commentId));
    return;
  }
...
async function deleteBoardComment(commentId) {
  ...
  try {
    ...
    await loadDetailComments(true);
    setStatus(document.getElementById('detailStatus'), '댓글이 삭제되었습니다.', 'success');
  } catch (err) {
    setStatus(document.getElementById('detailStatus'), err.message || '댓글 삭제에 실패했습니다.', 'error');
  }
}
